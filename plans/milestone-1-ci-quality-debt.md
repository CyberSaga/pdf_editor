...
- **Status:** merged 2026-07-04 as PR #18 (squash b538837; codex-reviewed, name-level DTO guard follow-up).
...
- **Triage decisions (verified against 4 downloaded junit reports + raw job logs, not just the hypothesis above):**
  - **Cluster A** (`test_no_jump_editor_geometry.py`, 13 failures) and **cluster B** (`test_core_interaction_audit.py::test_default_core_interaction_plan_uses_three_existing_fixtures`, 1 failure): confirmed fixture-dependent (`test_files/*.pdf` `.exists()` asserts or reads). Marked `@pytest.mark.needs_fixtures` at the individual-test level (not module-level — `test_no_jump_editor_geometry.py` has other synthetic-PDF tests that pass on CI today and must keep running there). 14 tests marked total; cross-checked against the CI FAILED list so no passing test was accidentally marked.
  - **Cluster C** (`test_performance_script_runner.py::test_performance_script_runs_from_repo_root`): **not** fixture-related — the CI traceback showed the *child* script (`test_performance.py`) itself throwing `UnicodeEncodeError` (`cp1252` can't encode its zh-TW status text) when its captured stdout isn't a real console. Windows-only (confirmed absent from the ubuntu junit failure list on the same runs). Fixed by forcing `PYTHONIOENCODING=utf-8` / `PYTHONUTF8=1` on the child's `env` in the test runner — a small test-harness fix, no mark needed.
  - **Cluster D** (`test_security_packaging.py::test_built_wheel_and_sdist_exclude_dev_trees`): the *original hypothesis above (charmap encode in the build) was wrong* — all 4 sampled runs (both OSes) show the real cause is `No module named build`: the `test-functional` CI job never installs the `build`/`setuptools`/`wheel` dev-extra trio the real-artifact test needs (`pyproject.toml`'s `dev` extra has them, but CI only installs `requirements.txt` + `optional-requirements.txt` + `pytest packaging`). Fixed by installing `build==1.5.0`/`setuptools==82.0.1`/`wheel==0.47.0` (pinned inline in `ci.yml`, matching the maintainer's `.venv`, since constraint files stay untouched in this PR) in the `test-functional` step. Separately hardened `test_security_packaging.py`'s subprocess decode from bare `text=True` (locale-dependent, reproduces the known local cp950 flake) to explicit `encoding="utf-8", errors="replace"` — same encoding class as the `optional-requirements.txt` cp1252 pitfall already in `docs/PITFALLS.md`.
  - **Cluster E** (ubuntu segfault): stays advisory. Real evidence (job logs, not the ~90%/"exit 139" guess above) shows a `Bus error` (SIGBUS, exit 135) inside `test_page_deskew_scope.py::test_controller_straightens_batch_as_single_undo` at a `qapp.processEvents()` call, at roughly the 49-58% mark through the suite, and it doesn't reproduce on every run (2 of 4 sampled runs completed the full ubuntu suite with ordinary failures and no crash). Filed as GitHub issue #19 with the corrected evidence; no fix attempted here.
- **CI selection change:** `test-functional`'s pytest invocation is now `-m "not local_only and not needs_fixtures"` on both legs (previously `not local_only` only, which is why clusters A/B failed instead of skipping).
- **Validation:** local `-m "not local_only and not needs_fixtures"` run collected 1586/1601 (15 deselected: 14 `needs_fixtures` + 1 pre-existing `local_only`), 1565 passed / 21 skipped / 0 failed. Full local suite (fixtures present, no filter) unchanged: 1580 passed / 21 skipped / 0 failed (confirmed identical on unmodified `main` @ b538837 — the marks do not alter local results; note this is the *current* main count, the milestone doc's earlier "1579" was one commit stale).
