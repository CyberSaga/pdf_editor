# TODOS -- Archive (completed items)

Moved from TODOS.md on 2026-07-02. Git history preserved at original path.
Active items remain in the root TODOS.md.
...
- [x] **R5.4 (2026-06-18) — Packaging guard test (no dev/test trees in shipped artifacts).** `scripts/` is a real package (`scripts/__init__.py`) holding the CUA pyautogui sign-off harness — it must never ship. New `test_scripts/test_security_packaging.py` guards both mechanisms: the wheel via `[tool.setuptools.packages.find].include` allow-list (assert no pattern matches scripts/test_scripts/docs) and the sdist via `MANIFEST.in` `prune scripts`/`prune test_scripts`, PLUS a best-effort real isolated `pip wheel` build asserting `_offending_members(names)` is empty (skips, not fails, if the build backend/network is unavailable — setuptools 57.4.0 in `.venv` can't build this PEP 621 project without isolation, but PyPI is reachable so isolation fetches a modern setuptools). **Teeth verified:** transiently adding `scripts*` to the discovery allow-list leaked 10 `scripts/` members into the wheel (incl. the gate/CUA harness), which the predicate flags. Also added `build/` to `.gitignore` (setuptools/`pip wheel` write it into the project root; the guard test cleans it up too) — consistent with the already-ignored `dist/` + `*.egg-info/`.
- [x] **R5.1 (2026-06-18) — Print path no longer writes a decrypted PDF to disk (HIGH).** Printing a password-protected PDF wrote a *fully decrypted* copy to `work_dir/input.pdf` (`capture_worker_snapshot_bytes` uses `PDF_ENCRYPT_NONE`; the worker wrote those bytes verbatim). **Option A:** `_PrintSubmissionWorker._encode_input_bytes()` re-encrypts with the session password (AES-256, `owner_pw==user_pw`) before the disk write, so the temp is never plaintext; the password reaches the helper **out-of-band** via the QProcess env (`PrintSubprocessRunner(helper_password=…)` → `PDF_EDITOR_PRINT_PASSWORD`, NOT `job.json` which sits in the same `work_dir`); `helper_main._build_snapshot_bytes(..., password=…)` authenticates the encrypted input in-memory so the printer still gets rasterizable bytes (decryption never on disk). Password captured only when `model.doc.needs_pass`; coordinator drops `self._print_password` once idle. Unencrypted/no-watermark fast path is byte-identical. All re-encrypt/auth saves live outside `model/`, so the encryption AST guard is untouched. Tests: `test_print_encrypted_input.py` (6: worker encrypts temp / plain-when-no-pw / password-not-in-job.json / helper decrypts-with-pw / raises-without-pw / unencrypted-unchanged).
- [x] **R5.5 (2026-06-18) — Optimize-copy preserves source encryption (HIGH).** 另存為最佳化的副本 of a password-protected PDF silently produced an *unprotected* copy: the encrypted (`needs_pass`) source rebuilds the working doc from decrypted `tobytes`, and the optimized output was saved with no encryption. `save_optimized_copy` now calls `reapply_source_encryption(model, new_path)` as the final step (after the move + any pikepdf post-packaging, which would itself strip encryption) — reopens the output and re-saves with the session password (`model.password`, `owner_pw==user_pw` since only one is retained), `int(doc.permissions)` (signed value round-trips exactly), and a method parsed from `metadata['encryption']` (default AES-256, never weakening). Detection: `doc.needs_pass` STAYS truthy after `authenticate()` in PyMuPDF 1.27 (`is_encrypted` flips instead), so it is the reliable post-auth "was encrypted" flag; owner-password-only docs (`needs_pass` False) are left as-is. The re-save targets a reopened output handle, never `model.doc`, so the R2.2 encryption AST guard stays green. Test: `test_pdf_optimize_workflow.py::test_save_optimized_copy_preserves_encryption`.
- [x] **R6.1 (2026-06-18) — Characterization tests for verified-untested live methods.** Four census-verified zero-reference model/controller surfaces with live production callers now have characterization tests pinning current behavior (the PyMuPDF/PySide6 glue that regresses silently across a version bump). `compose_merged_document` (`test_merge_composition.py`, 6: no-doc→ValueError, current/file/mixed-order inserts, skips unknown-kind + pathless-file, empty/None→0 pages); `get_print_watermarks` (`test_print_watermarks.py`, 4: empty, core fields, CJK round-trip, **deep-copy isolation** — teeth proven by confirming the shallow `get_watermarks` DOES leak a nested mutation while the deep print copy does not); the three worker→GUI bridge slots `forward_prepared`/`forward_succeeded`/`forward_status` + siblings across the print/optimize/OCR coordinators (`test_worker_bridge_slots.py`, 9: arity + payload identity across the `@Slot`→`Signal` hop, receivers bound to locals per the R5.1 GC gotcha); `get_text_selection_bounds` + the `run_reopen_anchors` session property (`test_text_selection_bounds.py`, 5: None on no-doc/out-of-range/miss, encloses selected line, session round-trip). 23 tests, ruff-clean. Characterization tests are green-by-construction (no red-light — they pin shipped behavior, not new features; PITFALLS entry added).
