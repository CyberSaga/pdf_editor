...
**Symptom:** Fractional font sizes (for example 9.5pt) silently become 9pt after editing.
...
**Symptom:** Text containing ligatures disappears after push-down; for example "misfits" becomes "mits".
...
## push-down insert_text(helv) drops non-Latin Unicode

**Area:** `model/pdf_model.py` -> `_push_down_overlapping_text`
**Symptom:** Pushed-down spans containing non-Latin characters or emoji are silently replaced.
**Cause:** Helvetica Type1 has no glyphs for many Unicode code points; PyMuPDF substitutes silently.
...
**Cause:** The original flow ran Strategy A (`insert_htmlbox`) on the main page to measure height, then cleared the full `insert_rect` with `apply_redactions()`. If `insert_rect` overlapped nearby horizontal content, the redraw erased it.
...
**Symptom:** Redo button is hidden behind the overflow menu.
**Cause:** The right block width was too small and the stretch widget squeezed the toolbar.
**Fix:** Increase the right block width, remove the stretch widget from `right_layout`, and give `toolbar_right` a minimum width.
**File:** `view/pdf_view.py`

---

## PDFModel has no `.open()` method
...
**Area:** test scripts
**Symptom:** Text preservation tests fail because the comparison target still uses ligature glyphs.
**Cause:** Test normalization stripped whitespace but did not expand ligature characters.
**Fix:** Add a ligature map and normalize before comparison.
...
**Fix:** Keep `__init__()` cheap (store refs only); put all view-signal wiring and startup sync in `activate()`.
**File:** `controller/pdf_controller.py`
