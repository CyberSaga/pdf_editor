...
**Fix:** Pass `encryption=fitz.PDF_ENCRYPT_KEEP` on every full-save-to-disk call (`_full_save_to_path` both branches; the `save_as` full-save/temp-overwrite branch). KEEP preserves whatever the source had — including "no encryption" — so it is safe to apply unconditionally on save-back/Save-As. Do **not** add it to internal snapshot saves (`_capture_doc_snapshot`, `_capture_page_snapshot*`): those are reopened later *without* a password, so they must stay decrypted, and they target fresh `insert_pdf` docs that carry no encryption anyway. Export paths (`new_doc.save`, `pix.save`) are deliberately new documents and out of scope.
