...
**Area:** model/text_commit/engine.py (`prepare` → `_build_scratch_copy`) / inspect.py (`page_fingerprint`)
**Symptom:** On `test_files/test-large-file.pdf`, `TieredCommitEngine.prepare()` fails its own scratch-first self-consistency check on the *first* attempt against any genuinely Tier-0-eligible target: the pre-scratch `page_fingerprint` never matches the scratch copy's.
**Cause:** PyMuPDF's `tobytes(encryption=KEEP)` re-serializes a disk-loaded object's dictionary with reordered keys (same keys, same length, different order) the first time through; it is idempotent after that one round trip. `page_fingerprint` hashes `xref_object()` strings verbatim, so a cosmetic reorder reads as a content change. Unit tests never hit this because synthetic fixtures are built via `fitz.open()` + `new_page()` — never loaded from disk. Scoped honestly: measured on this one file/producer; not yet confirmed universal.
**Fix:** Canonicalize once (a KEEP round trip) before the first fingerprint, or hash structured content rather than serialization strings. Workaround lives in `scripts/benchmark_text_commit_baseline.py` (`_canonicalize_once`); the production fix is a pre-Slice-1 TODOS item — with D1 landed, 5,853 shows on exactly this file are now eligible, so every real accept there hits this today.
**File:** `scripts/benchmark_text_commit_baseline.py`; production fix tracked in TODOS.md
