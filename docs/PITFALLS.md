...
**Fix:** Gate the round-trip on `not _doc_is_encrypted(doc)`, where `_doc_is_encrypted` reads the trailer's encryption string `(doc.metadata or {}).get("encryption")` — it stays populated after authentication and is set even for owner-only encryption, making it the reliable "was encrypted on disk" signal. An encrypted+damaged doc keeps MuPDF's in-memory-repaired (still encrypted, still file-backed) document; it can't save incrementally anyway, so save-back does a full rewrite with `encryption=KEEP`, which yields a clean xref (`is_repaired` clears on reopen) **and** preserves the password — verified end-to-end. `metadata` is only read on the damaged path (gated behind `is_repaired`), so healthy files pay nothing.
**File:** `model/pdf_model.py` (`open_pdf`, `_doc_is_encrypted`)
