...
## `page.transformation_matrix` does not reflect `/Rotate` on this PyMuPDF build — hardcoded to the unrotated flip matrix
**Area:** PyMuPDF 1.27.1 (installed `.venv`); affects any code reading `page.transformation_matrix` to detect page rotation
**Symptom:** `page.set_rotation(90)` (or setting `/Rotate` directly at the xref level) updates `page.rotation` and `page.rect` correctly, but `page.transformation_matrix` stays `Matrix(1, 0, 0, -1, 0, cropbox.height)` — the plain unrotated flip — regardless of rotation, even after a `tobytes()` + reopen round trip. Verified against the installed binding's own Python source: `if self.rotation % 360 == 0: val = <real MuPDF-computed transform> else: val = Matrix(1, 0, 0, -1, 0, self.cropbox.height)` — the `else` branch discards the real transform outright.
**Cause:** A binding-level simplification, not a caching or timing artifact — confirmed by baking `/Rotate` in from page creation (bypassing `set_rotation()` entirely) and by a full `tobytes()`/reopen round trip; both still return the unrotated matrix. `page.rotation_matrix` (a *different* property) does correctly reflect rotation.
**Fix:** Any new gate that needs to know whether a page's transform is a plain +x mapping (Task 11 Slice 1's growth-direction guard, `plan.py:_classify`) must check `page.rotation % 360 != 0` directly, not infer it from `page.transformation_matrix.a`/`.b` — matches the established pattern already at `pdf_content_ops.py:554` (matrix-skew check ALONGSIDE an explicit `rotation % 360` check, same underlying reason). Pre-Slice-1 code that reads `page.transformation_matrix` only for its *scale magnitude* (`math.hypot(a, b)`) is unaffected: `hypot` of the unrotated matrix's first row is still 1, the correct scale factor, regardless of rotation.
**File:** `model/text_commit/plan.py` (`_classify` growth-direction guard). Also exposed a test-fixture bug in `test_scripts/test_text_commit_tier1_transplant.py::test_growth_direction_unproven_refuses_rotated_page` — its fixture-sanity assertion read `matrix.b`/`matrix.a` directly and was unsatisfiable on this PyMuPDF build; the implementer reported it as a dispute (test files were immutable input for that role) and the orchestrator, after independently reproducing the PyMuPDF behavior, replaced the assert with a direct `rotation % 360 != 0` check

## `build_advance_preserving_erase`'s operator guard must admit `TJ`, unlike `build_transplant_replacement`'s
**Area:** model/text_commit/patch.py (Task 11 Slice 1 operator guards)
**Symptom:** The literal spec for both builders' guards ("refuse unless `show.operator == 'Tj'`") would, applied to `build_advance_preserving_erase`, have broken a pre-existing, pinned characterization test (`test_text_commit_textwriter_zorder.py::test_deleting_show_op_moves_later_text_then_erase_compensates_exactly`), which erases a `TJ`-array show (`[(ERA)-250(SEME)] TJ`) via this exact function and asserts success.
**Cause:** The two builders have different hazard profiles. `build_transplant_replacement` REPLACES an op with new content, where a `TJ` source array's per-item kerns would need to be understood and are not — the planner already refuses whole-`TJ` targets for exactly this reason (`NOT_SINGLE_LITERAL_TJ`), so no caller ever reaches this builder with one. `build_advance_preserving_erase` DELETES the op outright — a `TJ` array's internal kerns are irrelevant once the whole operator is gone, so it is exactly as safe to erase as a `Tj`. `'`/`"` are refused by BOTH builders regardless: their implicit `T*`/`Tw`/`Tc` side effects corrupt subsequent state whether the op is erased or replaced.
**Fix:** `build_advance_preserving_erase` refuses only `show.operator in ("'", '"')`; `build_transplant_replacement` refuses everything but `"Tj"` (nothing requires it to accept `TJ`, and the planner never routes one to it).
**File:** `model/text_commit/patch.py`

## V0c text extraction ignores content-stream clip paths — shared Tier 0/1 blind spot
**Area:** model/text_commit/verify.py (`_verify_commit` V0c, `get_text("text", clip=...)`)
**Symptom:** `page.get_text("text", clip=halo_rect)` reports text as extractable regardless of whether a `W n` clip path recorded in the content stream would hide it from a renderer — a clipped-away replacement can pass `replacement_extractable` even though nothing is visibly drawn.
**Cause:** MuPDF's clipped-text extraction honors the caller's `clip=` rectangle (a raster-space crop of the results) but not clip paths recorded *inside* the content stream; `replay.py` tracks `gs_depth` (nesting depth) only, never the clip rect a `q`/`W n`/`Q` scope established.
**Fix:** Not fixed — documented as a known, shared limitation across Tier 0 and Tier 1 (the same V0c check runs for both). The raster-based checks (V0d raster-identity-outside-halo, Tier 1's `growth_zone_is_uniform`) are unaffected: they operate on rendered pixels, where clipped-away content genuinely does not appear, so a zone or halo proven blank/identical in the raster is blank/identical regardless of clip paths.
**File:** `model/text_commit/verify.py` (`_verify_commit`); no test — the residual is in what V0c *cannot* detect

## Tier 1 growth widens the V0c extraction clip — a coincidental substring in a neighbor span can false-reject a valid commit
**Area:** model/text_commit/verify.py (`_verify_commit` V0c, declared-region halo)
**Symptom:** Tier 1 growth widens the extraction-clip halo from `target_bbox_page` to the declared region (target ∪ growth); a neighbor span whose bbox now brushes the wider clip, and whose own text happens to contain `original_text` as a substring, can trip the "source text still present" check even though the actual target's source text is gone.
**Cause:** V0c's "source gone" check (`prepared.original_text in clip_text`) has no per-span attribution — it greps the whole clipped extraction, so a coincidental substring match anywhere inside a wider clip reads the same as the real target surviving.
**Fix:** Not fixed — a new, Tier-1-only false-reject surface, safe direction (refuses a valid edit, never accepts a corrupt one). Growth zones are usually narrow (a few points of kern compensation), which keeps the added clip area small; noted for awareness, not mitigated in Slice 1.
**File:** `model/text_commit/verify.py` (`_verify_commit`)

## `growth_zone_is_uniform`'s narrow-zone skip can catch a neighbor glyph's anti-aliased edge
**Area:** model/text_commit/verify.py (`growth_zone_is_uniform`)
**Symptom:** A growth zone only a few points wide (~3pt at 96 DPI) is narrower than double the erosion margin, so erosion is skipped (mirroring `_region_is_uniform`'s own `ex0<=ex1 and ey0<=ey1` policy) and the raw, un-eroded pixels — including a neighboring glyph's anti-aliased fringe — are scanned, which can register as non-uniform and refuse `GROWTH_EXCEEDS_BLANK_REGION` for a zone that is visually blank.
**Cause:** Anti-aliasing blends a glyph's edge color into several pixels beyond its nominal boundary; erosion exists to skip that fringe, but a zone too narrow to erode has nowhere for the fringe to hide.
**Fix:** Not fixed — over-strict, safe direction (refuses growth that would have been fine; never admits growth that overwrites real ink). No new erosion policy invented for Slice 1; matches `_region_is_uniform`'s existing narrow-zone behavior exactly.
**File:** `model/text_commit/verify.py` (`growth_zone_is_uniform`)

## The growth-admission raster check proves uniformity, not blankness — name evidence accordingly
**Area:** model/text_commit/verify.py (`growth_zone_is_uniform`, `growth_zone_proven_uniform`)
**Symptom:** The Tier 1 growth-admission helper was first named `growth_zone_is_blank` and its verified property `growth_zone_proven_blank`, but the check compares every pixel in the zone to the first pixel — a uniformly DARK zone (a filled table cell, a redaction rectangle) passes exactly like white page background does. The evidence string overclaimed.
**Cause:** "Blank" conflates "no spatial detail" (what a single-color scan proves) with "matches the page background" (what it does not prove and does not need to: painting glyphs into a flat colored zone changes only pixels inside the declared region the outcome reports, and uniformity is the property that guarantees no pre-existing ink edge or glyph is being overwritten).
**Fix:** Renamed pre-ship — helper `growth_zone_is_uniform`, verified property `growth_zone_proven_uniform` — while the strings had zero telemetry exposure. The refusal code `GROWTH_EXCEEDS_BLANK_REGION` keeps its value: a refusal names the requirement conservatively and cannot overclaim evidence.
**File:** `model/text_commit/verify.py`

## Tier 1 widens Tier 0's pre-existing "equal advance ≠ equal ink height" exposure
**Area:** model/text_commit/verify.py (V0d raster-identity-outside-halo), model/text_commit/plan.py (target bbox formula)
**Symptom:** A replacement with taller ascenders or deeper descenders than the source (e.g. accented capitals at a large size) can poke outside the source bbox's vertical extent even with the 2pt halo margin applied, risking a false V0d "pixels changed outside the halo" failure.
**Cause:** Bbox height in both tiers is a fixed multiple of font size (`0.35*page_size` above baseline, `page_size` below), never the actual glyph outlines. Tier 0's equal-advance requirement never implied equal ink height either, but Tier 1 admits different-character replacements far more often — an equal-advance replacement is frequently the same characters reordered, a compensated-advance one commonly is not.
**Fix:** Not fixed — pre-existing Tier 0 exposure that fires more often at Tier 1, not a new defect class. Would need face-metric-derived vertical extents to mitigate; out of Slice 1 scope.
**File:** `model/text_commit/verify.py`, `model/text_commit/plan.py`
