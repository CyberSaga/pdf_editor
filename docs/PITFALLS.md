...
**Cause:** Measured on PyMuPDF 1.27.1 (no C source ships; reverse-measured against `mupdf.FzRect.fz_adjust_rect_for_stroke` / `mupdf.FzMatrix.fz_matrix_max_expansion`): `expand = (|w| or 1.0) * factor * max(|a|,|b|,|c|,|d|)`, with `factor = max(miterlimit, 0.5)` for miter joins and a flat `0.5` for round/bevel, plus the fixed 1.0 pt margin `fz_bound_text` always adds. `max|elem|` undershoots the largest singular value by up to √2 (a 45° rotation has every element at 0.707 while the pen is stretched by 1.0). Miter joins hide it under the 10× `miterlimit`; round/bevel joins do not, so `0.5·w·(σmax − max|elem|) > 1.0 pt` (e.g. `6 w 1 j` rotated 45° at any ctm scale above ~1.14) leaves stroked ink outside the O3 rect.
