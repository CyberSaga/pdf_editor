...
**Fix:** At plan time, scan the already-bounded page replay for another show with identical decoded bytes shown from a font that paints the same glyphs — resource names are aliases, so a second `/F_ALT` key pointing at the same font xref is the same painter and name equality misses it; so is a cloned dictionary whose only difference is a `/BaseFont` subset tag, which is why identity is xref OR equal evidence digest OR equal rendering evidence (subtype/encoding/embedded/advance source/widths/CID `W`+`CIDToGIDMap`+`DW`). Conclude "different" only when both capabilities were fully built: a degraded one (`advance_source == "none"`) proves nothing. Measure a proven candidate from its OWN text state (its capability's width for the text at its own size, its own Tc/Tw under the same codec branch the target uses, its own Tz) — never the target's advance scaled by a font-size ratio. Do NOT apply `Ts` to the candidate core: the planner refuses any target with a non-zero rise, so the target core is pinned to the baseline and a rise-translated candidate core opens a false-admit band at `rise >= 0.6*Tfs`. Reject strict overlap deeper than 0.05 pt on both axes; abutting shows and a genuinely disjoint 1 pt gap remain admissible. Anything unprovable (unresolvable resource, unproven width, or any `TJ`, whose array numerics replay drops so the recorded origin is not where the ink starts) is BOUNDED, not refused outright: `_painter_reach` gives the largest extent that can still land on the page (both directions for `TJ`), so a twin on another baseline stays admissible and only one that could reach the target core fails closed. The decoded-bytes prefilter keeps the scan bounded. XObject-hosted twins remain a replay blind spot bounded by V0d's raster halo.
...
**Fix:** After both raw advances are known, reject unless `Th`, both scaled advances, and the scaled delta are finite with `Th > 0`; reject any non-finite target bbox, and reject any non-finite `Tm`/`CTM` component at the same chokepoint — TRM shape admission inspects only the linear coefficients, so a non-finite TRANSLATION otherwise rides a finite caller bbox into Tier 1's derived geometry and surfaces as a late `verification_failed` instead of a stable planner reason. Derived boxes get their own check right after derivation, against the RASTER SAMPLER's precondition rather than mere finiteness: `verify._bbox_pixels` does `int(x * 96 / 72)` and raises `OverflowError` for finite coordinates above ~1.348e308, and a tiny `Tz` decouples the in-page growth strip from a background quad whose cross extent is `font_size` un-multiplied by `Th`, so every derived coordinate can be finite and still unrenderable. `_build_tier1` bounds them by `_RENDERABLE_COORD_LIMIT` (1e308 — the sampler's own precondition with headroom, NOT a tighter policy cut: a background box at 1.5e305 reaches the sampler and returns an ordinary rejection, so refusing it would be an admission regression). The Tier-1 `kern_for_displacement` call is wrapped separately, converting its `ValueError` into `unsupported_text_state`: the serialization chokepoint must keep raising, but `prepare` must keep returning reason codes. `kern_for_displacement` independently rejects non-finite displacement, font size, and result.
**File:** `model/text_commit/plan.py`; `model/text_commit/patch.py`; `test_scripts/test_text_commit_hscale_admission.py`
