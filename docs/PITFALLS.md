...
**Fix (structural, not per-call):** Pass `encryption=fitz.PDF_ENCRYPT_KEEP` on every full-rewrite-to-disk call (`_full_save_to_path` both branches; the `save_as` full-save/temp-overwrite branch). For in-memory round-trips, route **all** of them through a single chokepoint — `_roundtrip_live_doc(garbage=, deflate=)` — that always serializes with `encryption=KEEP` and re-authenticates the reopened handle (`_reauthenticate_if_needed`, using the in-memory `DocumentSession.password`). It also opens the new handle before closing the old, so a failed round-trip leaves the live doc intact. A regression test (`test_live_doc_tobytes_calls_preserve_encryption`) AST-scans the module and fails on any `self.doc.tobytes(...)` lacking `encryption=`, so the next instance can't ship. KEEP preserves whatever the source had (including "no encryption"), so it is always safe here. Do **not** add it to internal snapshot saves (`_capture_doc_snapshot`, `_capture_page_snapshot*`): those use `self.doc.save(stream)` on fresh `insert_pdf` docs and are reopened later *without* a password, so they must stay decrypted. Export paths (`new_doc.save`, `pix.save`) are deliberately new documents and out of scope.
**Known residual:** undo/redo of an encrypted doc round-trips through a *decrypted* in-memory snapshot (`_restore_doc_from_snapshot`), so after an undo the live doc is decrypted and a subsequent save loses encryption. Same invariant, but snapshots can't be re-encrypted without re-authentication; separate, pre-existing, not addressed here.
**File:** `model/pdf_model.py` (`_full_save_to_path`, `save_as`, `_roundtrip_live_doc`, `_maybe_garbage_collect`, `_repair_active_doc_in_memory`)
