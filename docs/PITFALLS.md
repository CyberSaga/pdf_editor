...
**Fix:** `compute_font_evidence_digest` folds the four resolved entry fields ahead of the per-subtype object closure. General rule for any evidence digest: fold the builder's INPUTS as the builder sees them (resolved values), not only the dictionaries you can name. The cross-document fingerprint (`inspect._update_font_dependencies`) has the analogous gap, recorded in TODOS.md — outside the revalidation fence.
**File:** `model/text_commit/fonts.py` (`compute_font_evidence_digest`); `test_scripts/test_text_commit_font_revalidation.py::test_indirect_basefont_target_rewrite_rebuilds_the_capability` (+ `_subtype_`, `_base_encoding_`)
