...
**Cause:** Cross-file Qt state pollution in one long-lived process: after hundreds of tests share a single offscreen `QApplication`, widget construction/drag-drop simulation can deadlock or abort in the C++ layer. It is nondeterministic and environment-sensitive (more likely in non-interactive/detached shells); no single test is at fault — per-file isolation makes every file green.
**Fix:** Verify the suite per-file: run each `test_scripts/test_*.py` in its own interpreter with a hard timeout (`timeout -k 10 240 .venv/Scripts/python.exe -m pytest -q <file>`), log one summary line per file, and aggregate. Hangs become named single-file timeouts instead of killing the run. Any monitor watching a long pytest run must treat "output file stale >5 min while process alive" as a terminal state alongside "summary printed" and "process gone".
**File:** no code change — harness procedure (sweep loop documented here; 2026-08-19 baseline: 220 files, 2473 passed / 21 skipped / 0 failed)
