# PDF 編輯器（PySide6 + PyMuPDF）

這是一個以 Python + Qt 開發的桌面 PDF 編輯器。

## 主要能力

- 開啟與儲存 PDF
- 可將一個或多個本機 PDF 直接拖曳到視窗上開啟成分頁（拖曳不會自動合併）
- 頁面操作：刪除、旋轉、插入、匯出、合併
- 操作物件模式：可移動/縮放/旋轉/刪除矩形與圖片，且允許物件彼此重疊
- 檔案頁籤提供最佳化副本輸出：內建 `快速` / `平衡` / `極致壓縮` 預設、可按需查看空間審計報告，並顯示容量縮減摘要
- 單一匯出對話框：可選當前頁/指定頁、DPI（`72` 到 `2400`）、輸出 PDF 或影像（`JPG/PNG/TIFF`）
- 重疊安全的文字編輯
- 文字編輯框若清空後提交，會刪除該文字框內容（可復原/重做）
- 獨立新增文字模式
- 文字屬性可用 `套用/取消` 明確提交或捨棄，且字體/字級選單不會誤關閉編輯框
- 擴充 CJK 字體選項，支援 `Microsoft JhengHei`、`PMingLiU`、`DFKai-SB`（可用時嵌入字型）
- 註解：矩形、螢光、文字註解
- 浮水印
- 搜尋與跳轉
- 可選 OCR
- Windows 列印具備韌性：使用者確認列印後才開始背景 snapshot/輸入捕捉，送印（raster submission）改由 helper subprocess 隔離以避免主視窗沒有回應；列印期間關閉會延後處理並在完成後自動關閉；若子系統無回應可終止背景列印工作而不需重啟主視窗
- 列印對話框在支援的平台上，印表機選單旁提供原生屬性按鈕（`屬性`），並將屬性視窗回傳的紙張/方向/雙面/色彩/DPI/份數同步回對話框欄位；紙盤與廠商私有設定直接沿用系統屬性，不再於 app UI 顯示紙盤/系統屬性區塊
- 列印硬體欄位採 touched precedence：`紙張`、`方向`、`雙面`、`色彩` 只有在使用者於 app 內明確變更時才覆寫系統 `屬性`；`份數`、`DPI`、`逐份列印`、頁面範圍與縮放仍屬於 app 端工作設定
- 列印預覽對暫時性的無效輸入（例如空白的自訂頁碼）會在對話框內顯示預覽錯誤，而不會從 UI 事件路徑直接拋出例外；整份列印 snapshot 也改為在使用者按下列印確認後才生成
- 透過命令歷史支援 Undo/Redo

## 文件地圖

- 功能行為：`docs/FEATURES.md`
- 架構與邊界：`docs/ARCHITECTURE.md`
- 問題歷程與修正：`docs/solutions.md`
- 測試腳本索引：`test_scripts/TEST_SCRIPTS.md`

## 環境需求

- Python 3.10+
- Windows（主要開發環境）

本專案使用的第三方套件：
- PySide6
- PyMuPDF (`fitz`)
- Pillow
- pytesseract（選用）

## 安裝

```text
python -m venv .venv
.venv\Scripts\python -m pip install -r requirements.txt
```

選用相依套件：

```text
.venv\Scripts\python -m pip install -r optional-requirements.txt
```

OCR 說明：請安裝 Tesseract OCR，並確保 `tesseract` 可由 PATH 找到。

## 執行

```text
python main.py
```

## 打包（Windows）

```text
.venv\Scripts\python -m PyInstaller --noconfirm --clean --onefile --windowed main.py
```

輸出：

```text
dist\main.exe
```

## 快捷鍵

- `Ctrl+O`：開啟 PDF
- `Ctrl+P`：開啟列印對話框
- `Ctrl+Alt+L`：開關左側邊欄
- `Ctrl+Alt+R`：開關右側邊欄
- `Ctrl+Z`：復原
- `Ctrl+Y`：重做
- `Ctrl+S`：存檔
- `Ctrl+Shift+S`：另存新檔
- `F2`：進入文字編輯模式
- `F5`：切換全螢幕
- `Esc`：優先關閉目前編輯框/對話框；若無且非瀏覽模式則切回瀏覽模式

未儲存變更的關閉提醒支援：
- `Y`：儲存
- `N`：放棄
- `Esc`：取消

## 授權與致謝

本專案採用 **GNU 通用公共授權條款第三版 (GPLv3)**。

### 第三方致謝

本專案使用了 **Surya** (由 Vik Paruchuri 開發) 提供的 OCR 功能。
- **程式碼：** 採用 GPLv3 授權。
- **模型權重：** 採用 **AI Pubs Open Rail-M** 授權。對於個人使用、研究，以及年營收與總資助額低於 500 萬美元的新創公司免費。若超過此門檻之商業用途，需向原作者 (Datalab) 取得商業授權。

## 免責

本軟體以「現況」提供，不提供任何明示或默示保證。
