...
- **Encryption preservation (R5.5).** The optimize-copy working doc is rebuilt from the *decrypted* live-doc bytes (`build_working_doc_for_optimized_copy` → `tobytes` for the encrypted/`needs_pass` source), so the optimized output would otherwise ship unprotected. `save_optimized_copy` calls `reapply_source_encryption(model, new_path)` as the final step (after the move, after any pikepdf post-packaging that would itself strip encryption): when `model.doc.needs_pass` is truthy it reopens the output and re-saves with the session password (`model.password`, `owner_pw == user_pw` — only one password is retained), the live doc's permission bits (`int(doc.permissions)`), and a method parsed from `metadata['encryption']` (default AES-256, never weakening). The re-save targets a reopened handle of the output file, never `model.doc`, so the R2.2 encryption AST guard does not flag it. Owner-password-only PDFs (`needs_pass` False) are left unencrypted, matching how the live session opens them.
