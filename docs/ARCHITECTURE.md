...
| 檔案 | 職責 |
|------|------|
| `view/pdf_view.py` | 主視窗、工具列（儲存/開啟/另存/刪除頁/旋轉頁/匯出頁、插入空白頁/從檔案插入頁、瀏覽/縮圖/搜尋、編輯文字/矩形/螢光筆、新增註解/註解列表、浮水印、顯示隱藏註解、OCR、快照、復原/重做）；左側縮圖、中央 QGraphicsView 連續捲動畫布、右側繪製設定（矩形顏色與透明度、螢光筆顏色、字型與字級、垂直文字選項）；搜尋面板、註解列表、浮水印列表、右鍵選單；發出各類 Signal 給 Controller |
