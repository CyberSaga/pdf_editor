...
- Window/taskbar identity: `main.py` sets AppUserModelID `CyberSaga.CyberSagaPDF` and the application icon (`view/resources/app_icon.ico`), so the taskbar shows the app's own name/icon instead of a generic Python entry.
...
On Windows, vendor/private properties set in `屬性` are applied **job-scoped**, not persisted: the captured DEVMODE is carried with the job (base64 in `extra_options`) and applied only for that print run by briefly writing the per-user default (level 9) and restoring the previous default afterwards. A single print no longer permanently mutates the printer's defaults for other jobs or applications. On the rarer pywin32-only fallback path the opaque DEVMODE cannot be serialized, so only the public fields (paper, orientation, duplex, colour, tray) are carried via the dialog controls.
On Windows the spooler renders each page to a raster bitmap; to keep the GDI/EMF spool small the effective raster resolution is capped (currently 150 DPI) for the real printer path, while PDF-output/virtual-printer targets keep full DPI. Because the GDI spooler ignores mid-job page-layout changes, a document whose pages have mixed sizes/orientations is split into one spooler job per contiguous uniform-layout group so each page prints on its correct media; multi-copy collated jobs loop the whole document in order across those groups (a single physically-collated set spanning paper sizes is not possible on GDI, so such a job is not perfectly atomic — a mid-job failure reports how many pages were already spooled).
