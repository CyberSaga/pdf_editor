...
| 6 | F1 | in_progress | App-owned object manipulation is partially landed: objects mode, same-page multi-select, resize handles, and app-inserted images are now in place, while later native-PDF image follow-ups stay open. |
...
| F1 | Add/manipulate objects (including rotate text box) | 6 | in_progress | done-implement | Active object-mode plans at `docs/plans/2026-04-13-f1-object-manipulation-v1.md`, `docs/plans/2026-04-14-f1-objects-mode-v2.md`, and `docs/plans/2026-04-14-f1-objects-mode-v3.md`; focused object slices are green in `test_object_requests.py`, `test_object_manipulation_model.py`, `test_object_controller_flow.py`, `test_object_manipulation_gui.py`, `test_object_multi_select.py`, `test_object_resize.py`, `test_image_objects_model.py`, `test_image_objects_gui.py`, and `test_interaction_modes.py`; live mixed-sample GUI verification now passes via `tmp/manual_verify_f1_qtest.py` | Finish the remaining native-PDF image follow-up and keep later F1 polish in separate child plans. |
