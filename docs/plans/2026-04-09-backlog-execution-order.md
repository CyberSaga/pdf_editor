...
| 6 | F1 | done | Object manipulation is now complete for the current backlog scope: objects mode, same-page multi-select, resize handles, app-inserted images, and native PDF image manipulation are all landed. |
...
| 9 | F4, F2 | open | Replace with child plans before implementation. `F3` open-APIs slice is now implemented; OS registration remains intentionally out of scope. |
...
| F1 | Add/manipulate objects (including rotate text box) | 6 | done | done-implement | Active object-mode plans at `docs/plans/2026-04-13-f1-object-manipulation-v1.md`, `docs/plans/2026-04-14-f1-objects-mode-v2.md`, and `docs/plans/2026-04-14-f1-objects-mode-v3.md`; focused object slices are green in `test_object_requests.py`, `test_object_manipulation_model.py`, `test_object_controller_flow.py`, `test_object_manipulation_gui.py`, `test_object_multi_select.py`, `test_object_resize.py`, `test_image_objects_model.py`, `test_image_objects_gui.py`, `test_native_pdf_images_model.py`, and `test_interaction_modes.py`; live mixed-sample GUI verification passes via `tmp/manual_verify_f1_qtest.py`, and a real-file native-image round-trip on `test_files/2.pdf` now verifies move/rotate/delete plus save/reopen through the model path. | F1 is closed; keep any later object-polish work in separate child plans. |
...
| F3 | File-explorer right-click / merge with PDF Editor | 9 | done | done-implement | `pytest -q test_scripts/test_cli_argparse.py test_scripts/test_headless_merge.py test_scripts/test_single_instance_forwarding.py` plus `pytest -q test_scripts/test_main_startup_behavior.py -k "empty_launch or cli_open_path"`; `main.py` now supports positional file args, headless `--merge`, and per-user single-instance argv forwarding without touching registry or file associations | F3 is closed for the user-approved scope. Keep OS-level shell registration, context-menu verbs, and installer integration out of scope unless a later child plan reopens them. |
| F4 | Color profile switching (view-only) | 9 | done | done-implement | View-only colorspace switching (sRGB/灰階/CMYK 預覽) is now implemented end-to-end: controller threads `SessionUIState.color_profile` through pixmap rendering (pages/thumbnails/snapshots) and the Windows helper raster print path via `PrintJobOptions.extra_options["render_colorspace"]`. Focused coverage: `pytest -q test_scripts/test_color_profile_enum.py test_scripts/test_render_colorspace.py test_scripts/test_color_profile_controller.py test_scripts/test_color_profile_gui.py test_scripts/test_print_controller_flow.py test_scripts/test_print_colorspace.py`. | Document-level ICC conversion/export mutation remains intentionally deferred. |
