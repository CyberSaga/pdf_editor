...
| 4 | B1, UX4, UX5 | open | Keep under `src/printing/` and preserve `override_fields` contract. |
...
|---|---|---|---|---|---|---|
| B1 | Print must not modify printer preferences | 4 | open | done-implement | Existing regression coverage in `test_win_driver_properties.py` and `test_linux_driver_overrides.py` | Add red tests for untouched hardware fields, then patch drivers/bridge. |
...
| UX4 | Print auto-rotate based on page orientation | 4 | open | done-implement | Print dialog/bridge tests already cover layout resolution | Add inference tests and mark `orientation` overridden only when auto-inferred. |
| UX5 | Print auto-select paper size from source page | 4 | open | done-implement | Print dialog logic script already resolves paper size | Add unambiguous-size inference tests and mark `paper_size` overridden only when inferred. |
| UX6 | Text selection should use whole-line units | 3 | done | done-implement | `pytest -q test_text_extraction_line_joining.py -k "get_text_in_rect_expands_partial_clip_to_whole_visual_lines or get_text_bounds_expands_partial_clip_to_full_visual_line_bounds"` | Keep browse-mode selection snapped to full visual line units while Phase 4 print work lands. |
