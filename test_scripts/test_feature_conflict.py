...
def run_page_ops(pdfs: list[Path]) -> ConceptResult:
    ...
    for p in multi:
        ...
        try:
            ...
            r.cases.append(
                CaseResult(
                    p.name, len(err) == 0, _ms() - t0, f"n0={n0}", "; ".join(err)
                )
            )
        except Exception as e:
            r.cases.append(CaseResult(p.name, False, _ms() - t0, "", str(e)[:150]))
...
def run_edit_undo_redo(pdfs: list[Path]) -> ConceptResult:
    ...
    for p in pdfs[:8]:
        ...
        try:
            ...
            m.close()
            r.cases.append(
                CaseResult(p.name, len(err) == 0, _ms() - t0, "", "; ".join(err))
            )
        except Exception as e:
            r.cases.append(CaseResult(p.name, False, _ms() - t0, "", str(e)[:150]))
    r.total_ms = sum(c.duration_ms for c in r.cases)
...
def run_annot_rect_highlight(pdfs: list[Path]) -> ConceptResult:
    ...
    for p in pdfs[:6]:
        ...
        try:
            ...
            page = m.doc[0]
            r0 = page.rect
            m.tools.annotation.add_annotation(
                1, fitz.Point(r0.x0 + 50, r0.y0 + 50), "F4 annot"
            )
            m.tools.annotation.add_rect(
                1, fitz.Rect(100, 100, 200, 150), (1, 0, 0, 0.5), False
            )
            m.tools.annotation.add_highlight(
                1, fitz.Rect(100, 160, 250, 180), (1, 1, 0, 0.5)
            )
...
def run_watermark(pdfs: list[Path]) -> ConceptResult:
    ...
    for p in pdfs[:4]:
        ...
        try:
            ...
            m.open_pdf(str(p), password=_get_password(p))
            n = len(m.doc)
            m.tools.watermark.add_watermark(
                list(range(1, min(3, n) + 1)),
                "F6 WM",
                45,
                0.4,
                36,
            ...
            if not wl:
                r.cases.append(
                    CaseResult(p.name, False, _ms() - t0, "", "get_watermarks empty")
                )
                m.close()
                continue
            wid = wl[0].get("id")
            m.tools.watermark.update_watermark(
                wid,
                pages=[1],
                text="F6 updated",
                angle=30,
            ...
            r.cases.append(
                CaseResult(
                    p.name, ok, _ms() - t0, f"before={len(wl)} after={len(wl2)}", ""
                )
            )
        except Exception as e:
            r.cases.append(CaseResult(p.name, False, _ms() - t0, "", str(e)[:150]))
...
def run_conflict_annot_then_edit(pdfs: list[Path]) -> ConceptResult:
    ...
    for p in pdfs[:6]:
        ...
        try:
            ...
            rect = page.rect
            m.tools.annotation.add_annotation(
                pi + 1, fitz.Point(rect.x0 + 60, rect.y0 + 60), "C1 annot"
            )
            m.edit_text(
                pi + 1,
                blk.layout_rect,
            ...
            r.cases.append(
                CaseResult(
                    ...
                    ok,
                    _ms() - t0,
                    f"annots after edit={after_ann}",
                    "" if ok else "annot lost",
                )
            )
        except Exception as e:
            r.cases.append(CaseResult(p.name, False, _ms() - t0, "", str(e)[:150]))
...
def run_save_with_watermark(pdfs: list[Path]) -> ConceptResult:
    ...
    for p in pdfs[:4]:
        ...
        try:
            ...
            m.open_pdf(str(p), password=_get_password(p))
            m.tools.watermark.add_watermark(
                [1], "C6 WM", 45, 0.3, 24, (0.8, 0.8, 0.8), "helv"
            )
            with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as f:
                tmp = f.name
            m.save_as(tmp)
            ...
            r.cases.append(
                CaseResult(
                    ...
                    _ms() - t0,
                    f"watermarks={len(wl)}",
                    "" if ok else "get_watermarks empty or render failed",
                )
            )
        except Exception as e:
            r.cases.append(CaseResult(p.name, False, _ms() - t0, "", str(e)[:150]))
...
def generate_report(concepts: list[ConceptResult], total_ms: float) -> str:
    ...
    lines.append("\n【總覽】")
    lines.append(
        f"  概念數：{len(concepts)}  案例數：{total_cases}  通過：{total_passed}  失敗：{total_failed}"
    )
    ...
    lines.append("【各概念摘要】")
    lines.append(
        f"  {'ID':<6} {'標題':<42} {'通過':<6} {'失敗':<6} {'通過率':<8} {'耗時'}"
    )
    for c in concepts:
        fail = c.total - c.passed
        flag = "✓" if fail == 0 else "✗"
        lines.append(
            f"  {c.id:<6} {c.title[:42]:<42} {c.passed:<6} {fail:<6} {c.pass_rate:6.1f}%  {c.total_ms / 1000:.2f}s  {flag}"
        )
...
def main():
    ...
    if not pdfs:
        print(
            "  未找到可用的測試 PDF，請確認 test_files/sample-files-main 存在且含 PDF。"
        )
        return 1
    ...
    total_passed = sum(c.passed for c in concepts)
    print(
        f"  總案例：{total_cases}  通過：{total_passed}  失敗：{total_cases - total_passed}"
    )
