...
class TrackAEngine:
    ...
    def execute_reflow(
        ...
        try:
            ...
            # Redact 被編輯的塊（用 tight bbox 覆蓋實際文字區域）
            page.add_redact_annot(tight_rect)
    ...
    def _estimate_text_height(
        ...
        """
        估算文字在指定寬度內的排版高度。
        簡化計算：依字元數 / 每行字元數 估算行數 × 行高。
        """
        if not text:
            return 0.0

        # 粗估每行字元數（假設等寬字，實際依字體不同）
        avg_char_width = size * 0.6   # 粗估
    ...
    def _reflow_affected_blocks(
        ...
        # 先收集所有受影響塊的文字（redact 前，用 old_bbox 比對而非 block number）
        # 說明：edited block redact+re-insert 後 PyMuPDF 重排 block number，
        #       舊 number 已失效；改用 old_bbox 做近鄰比對（容忍 1pt 偏差）。
        text_dict = page.get_text("dict", flags=fitz.TEXT_PRESERVE_WHITESPACE)
        all_blocks = text_dict.get("blocks", [])
        block_texts = {}

        for block_idx, old_bbox, new_bbox in plan.affected_blocks:
            # 以 old_bbox 中心比對當前 block 位置
        ...
        for block_idx, old_bbox, new_bbox in plan.affected_blocks:
            ...
            rc = page.insert_htmlbox(new_bbox, info["text"], css=css)
            if (rc[0] if isinstance(rc, tuple) else rc) < 0:
                plan.warnings.append(
