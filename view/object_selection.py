...
class ObjectSelectionManager:
    ...
    def _clear_object_selection(self) -> None:
        self._view._selected_object_info = None
        if hasattr(self._view, "_selected_object_infos"):
            self._view._selected_object_infos = {}
        if hasattr(self._view, "_selected_object_page_idx"):
            self._view._selected_object_page_idx = None
        self._view._object_drag_pending = False
        self._view._object_drag_active = False
        self._view._object_rotate_pending = False
        self._view._object_drag_start_scene_pos = None
        self._view._object_drag_start_doc_rect = None
        self._view._object_drag_preview_rect = None
        self._view._object_drag_page_idx = None
        if self._view._object_selection_rect_item is not None:
            try:
                self._view.scene.removeItem(self._view._object_selection_rect_item)
            except Exception:
                pass
            self._view._object_selection_rect_item = None
        if self._view._object_rotate_handle_item is not None:
            try:
                self._view.scene.removeItem(self._view._object_rotate_handle_item)
            except Exception:
                pass
            self._view._object_rotate_handle_item = None
        for item in getattr(self._view, "_object_resize_handle_items", []) or []:
            try:
                self._view.scene.removeItem(item)
            except Exception:
                pass
        self._view._object_resize_handle_items = []
        self._view._object_resize_pending = False
        self._view._object_resize_active = False
        self._view._object_resize_start_scene_pos = None
        self._view._object_resize_start_doc_rect = None
        self._view._object_resize_preview_rect = None
        self._view._object_resize_handle_anchor = 3  # default BR

    def _select_object(self, info) -> None:
        self._view._selected_object_info = info
    ...
    def _rebase_object_selection_to_bboxes(self, new_bboxes: dict[str, fitz.Rect]) -> None:
        ...
        """
        infos = getattr(self._view, "_selected_object_infos", None)
        selected = self._view._selected_object_info
        ...
        for oid, new_bbox in new_bboxes.items():
            ...
            if infos is not None and oid in infos:
                infos[oid] = new_info
            if selected_oid == oid:
                self._view._selected_object_info = new_info
        if infos:
            self._view._object_drag_start_doc_rects = {
                k: fitz.Rect(v.bbox) for k, v in infos.items()
            }
        if self._view._selected_object_info is not None:
            self._view._object_drag_start_doc_rect = fitz.Rect(self._view._selected_object_info.bbox)
            self._view._object_drag_preview_rect = fitz.Rect(self._view._selected_object_info.bbox)
        self._view._update_object_selection_visuals()
    ...
    def _apply_object_selection_rotation(self, angle_deg: float) -> None:
        ...
        center = getattr(self._view, "_object_rotate_center_scene", None)
        if center is None:
            return
        items = [getattr(self._view, "_object_selection_rect_item", None)]
        items.append(getattr(self._view, "_object_rotate_handle_item", None))
        items.extend(getattr(self._view, "_object_resize_handle_items", None) or [])
        for item in items:
            if item is None:
                continue
            try:
    ...
    def _update_object_selection_visuals(self, rect: fitz.Rect | None = None) -> None:
        info = getattr(self._view, "_selected_object_info", None)
        ...
        # a freed object.
        if self._view._object_selection_rect_item is not None and not shiboken6.isValid(self._view._object_selection_rect_item):
            self._view._object_selection_rect_item = None
        if self._view._object_rotate_handle_item is not None and not shiboken6.isValid(self._view._object_rotate_handle_item):
            self._view._object_rotate_handle_item = None
        if getattr(self._view, "_object_resize_handle_items", None):
            self._view._object_resize_handle_items = [
                item for item in self._view._object_resize_handle_items if shiboken6.isValid(item)
            ]
        ...
        if self._view._object_selection_rect_item is None:
            self._view._object_selection_rect_item = self._view.scene.addRect(scene_rect, pen, brush)
            self._view._object_selection_rect_item.setZValue(21)
        else:
            self._view._object_selection_rect_item.setRect(scene_rect)
            self._view._object_selection_rect_item.setPen(pen)
            self._view._object_selection_rect_item.setBrush(brush)
        if self._view._supports_free_rotate(info):
            handle_rect = QRectF(scene_rect.right() - 12, scene_rect.top() - 18, 12, 12)
            if self._view._object_rotate_handle_item is None:
                self._view._object_rotate_handle_item = self._view.scene.addEllipse(
                    handle_rect,
                    QPen(QColor(2, 132, 199, 230), 1),
                    QBrush(QColor(56, 189, 248, 220)),
                )
                self._view._object_rotate_handle_item.setZValue(22)
            else:
                self._view._object_rotate_handle_item.setRect(handle_rect)
        elif self._view._object_rotate_handle_item is not None:
            try:
                self._view.scene.removeItem(self._view._object_rotate_handle_item)
            except Exception:
                pass
        ...
        if getattr(self._view, "_object_resize_handle_items", None) is None:
            self._view._object_resize_handle_items = []
        for item in list(self._view._object_resize_handle_items):
            try:
                self._view.scene.removeItem(item)
            except Exception:
                pass
        ...
        for hx, hy in (
            ...
            try:
                ...
                pass
            self._view._object_resize_handle_items.append(item)
    ...
    def _hit_object_resize_handle_index(self, scene_pos: QPointF) -> int:
        ...
        items = getattr(self._view, "_object_resize_handle_items", None) or []
    ...
    def _point_hits_object_rotate_handle(self, scene_pos: QPointF) -> bool:
        if self._view._object_rotate_handle_item is None:
            return False
        try:
            return self._view._object_rotate_handle_item.rect().contains(scene_pos)
        except Exception:
            return False
    ...
    def _commit_free_rotation(self) -> bool:
        ...
        if not self._view._supports_free_rotate(info):
            return False
        start_rotation = float(getattr(self._view, "_object_rotate_start_rotation", 0.0) or 0.0)
        start_angle = float(getattr(self._view, "_object_rotate_start_angle", 0.0) or 0.0)
        delta_screen = float(getattr(self._view, "_object_rotate_preview_angle", 0.0) or 0.0)
        ...
        self._view._object_rotate_preview_angle = 0.0
        # Clear the live preview transform; the page re-render + reselect will
        # rebuild the frame around the new (rotated) bounding box.
        for item in (
            [getattr(self._view, "_object_selection_rect_item", None),
             getattr(self._view, "_object_rotate_handle_item", None)]
            + (getattr(self._view, "_object_resize_handle_items", None) or [])
        ):
            if item is None:
                continue
            try:
                ...
                continue
        self._view._selected_object_info = replace(
