...
- [x] Task 12 as planned — **sealed 2026-08-14** (plan archived to `plans/archive/2026-08-12-task12-engine-hardening.md`; P0-A/B/C/D merged via PRs #29/#30/#31, Step 7–8 cleanup via the `task12/closure-cleanup` PR). Rollout gates remain OPEN by design: defaults are still `legacy`/`max_tier=0`, and the three corpus blockers (marked-content tolerance, rotated-Tm, replay-budget latency) move to `plans/task13-cad-binding-unlock.md` — nothing reaches a user until those gates pass
