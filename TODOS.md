...
- [ ] **D5 — direct tests for `_tier0_target_from_resolve` (`pdf_text_edit.py:1185-1230`), FIRST.** Zero direct tests exist; the only test contact stubs its wrapper out (`test_text_commit_preview_contract.py:508`), and PITFALLS records this exact stage once reduced Tier 0 to "never fires". Must pin: the `" ".join` reconstruction (`:1223`) byte-matches `bind_source_text`'s exact-equality demand (`inspect.py:233`) — multi-space/tab/kern-derived gaps currently become silent `NO_MATCH` above the planner, invisible to every corpus number. Covers the open `MULTI_SPAN_TARGET` gap (same harness). Until this exists, every coverage % is unfalsifiable
