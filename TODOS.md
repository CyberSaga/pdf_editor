...
- [ ] **NEW (baseline stage, 2026-08-01): scratch-first fingerprint self-check fails on disk-loaded documents — blocks every real Tier 0 accept on the dominant file, fix before Slice 1.** `tobytes(encryption=KEEP)` reorders a disk-loaded object's dictionary keys on first serialization (idempotent after one round trip); `page_fingerprint` hashes `xref_object()` strings verbatim, so `TieredCommitEngine.prepare()`'s pre-scratch fingerprint never matches the scratch's — measured on `test_files/test-large-file.pdf`, the exact file where D1 just made 5,853 shows eligible. Unit tests never see it (synthetic fixtures are never disk-loaded). Fix Red-light-first: canonicalize once before the first fingerprint, or fingerprint structured content; workaround shape in `scripts/benchmark_text_commit_baseline.py:_canonicalize_once`. Until fixed, D1's 15.19% is not merely headroom — it is unreachable in practice on real files
...
- [ ] **Fallback `target_bbox` still assumes left-to-right text in page space** (D1 fixed the Tm scale term, the Codex-P2 fix above fixed the `/UserUnit` page-scale term — both are now correct at any uniform scale): the *shape* is still wrong on `/Rotate 90`/`270` pages independent of scale — box is built axis-aligned (`[x0, y0-size]..[x0+advance, y0+0.35*size]`) while glyphs run vertically in page space; `origin_page` itself is correct (goes through the full transform), only the box shape is not, so V0c would false-reject. Reachable from the preview path (`request.target_bbox` is `tuple | None`, forwarded by `text_commit/preview.py`). Production is safe today only because `pdf_text_edit.py:1225` passes a real page-space bbox. Fix needs a rect-in-user-space-then-transform formulation (fixes scale and shape together) plus a `/Rotate` fixture — deliberately not attempted alongside the scale fix to avoid an untested behavior change on rotated pages; see PITFALLS
