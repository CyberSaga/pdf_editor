...
- [ ] **NEW (measurement stage, 2026-08-01): make `TARGET_IN_FORM_XOBJECT` target-scoped in production `bind_source_text`.** The label fires on any miss on any page invoking any XObject (98.6% of corpus pages); only 5.3% of firings are confirmed target-in-XObject. Port the funnel script's per-target deconfliction (replay each invoked Form XObject once per doc, byte-check the target) — or stop reading the code in telemetry/audits until then. See PITFALLS
...
### During Task 11
- [x] **Slice 1 shipped (2026-08-09, by-fable worktree, Red-light first).** `[(newtext) K] TJ` transplant at the source `Tj` op's exact byte range, same font resource/encoding, `K` absorbs the advance delta. `test_scripts/test_text_commit_tier1_transplant.py` — 20 tests incl. the composite anchor (renders, kern-compensated, every later show retains origin, persistent text state unchanged, exact splice range + stream digest, preview token == commit token, undo byte-identical, forced-verification-failure reverts, honest outcome, save/reopen persists) — all 22 green after review closure (20 original + live-growth-recheck divergence + command-level Tier 1 undo). Flag-off by default (`TEXT_COMMIT_MAX_TIER=0`). Design + adjudication: `plans/2026-08-09-task11-slice1-by-fable.md`; architecture summary: `docs/ARCHITECTURE.md` §10.2.
  - **Implementer dispute, adjudicated and resolved:** `test_growth_direction_unproven_refuses_rotated_page`'s fixture-sanity assertion read `page.transformation_matrix` and was unsatisfiable on the installed PyMuPDF 1.27.1 (the property hardcodes the unrotated flip matrix whenever `page.rotation != 0` — verified against the binding's own source; see PITFALLS). The guard itself was correct and mutation-verified; only the test's premise was false. The orchestrator (with independent verification of the PyMuPDF claim) replaced the assert with `rotation % 360 != 0` plus a comment recording the quirk — the implementer honored the test-immutability rule and reported instead of editing, which is the intended protocol.
  - **Adversarial review (opus, verdict approve, 3 minors — all fixed pre-commit since nothing had shipped):** `growth_zone_is_blank`/`growth_zone_proven_blank` renamed to `growth_zone_is_uniform`/`growth_zone_proven_uniform` (the raster proof is uniformity, not blankness — a uniformly dark zone passes; see PITFALLS); page-boundary refusal narrowed to the growth edge (`x1` only — the source's own y-extent may legitimately poke past the margin on near-edge fallback bboxes, and refusing on it false-rejected purely horizontal growth); the per-keystroke O(page_count) shared-stream scan now cached once per preview session (`PlanPreviewRenderer` passes `shared_stream_xrefs` into `prepare_plan`); dead `-0.0` kern normalization removed (unreachable: Tier 1 entry requires the delta to exceed the Tier 0 tolerance, so |K| >= ~1e-6); `_rejection_outcome` fallback-chain labels carry the candidate's real tier (`tier1:...`, was hardcoded `tier0:`). Mutation battery 8/8 SENSITIVE: live growth re-check, Tier 1 reversal admission, transplant operator guard, font_size gate, shared-stream gate, page-boundary gate, rotation clause, kern sign.
- [x] **Tier 1 contracts that Task 11's file list did not originally cover** (GPT-5.6-sol) — all landed in Slice 1: `PreparedEdit` tier/kern/growth DTOs, preview↔commit candidate identity (token equality, pinned test), live-commit rollback (`engine.commit`'s growth check + verify-failure revert), persistence/undo/redo (`build_reversal_patchset` is tier-agnostic; post-review, `EditTextCommand.execute` admits Tier 1 to the byte-exact reversal capture so command-level undo does not fall through to the lossier page-snapshot path — pinned by `test_tier1_command_undo_replays_byte_exact_reversal`), resource-dictionary reuse (`build_tier1_font_outcome`, xref-proven), and growth-region semantics (`growth_bbox_page` + `growth_zone_is_uniform`). **Shared content streams** now refused on the common path for both tiers (`RejectReason.SHARED_CONTENT_STREAM`, `inspect.scan_shared_streams`) — closes the latent Tier 0 hole too.
- [x] **Operator guard on `patch.py:156-213`** — both builders now check `show.operator`, but NOT symmetrically: `build_transplant_replacement` refuses anything but `Tj`; `build_advance_preserving_erase` additionally accepts `TJ` (erasing a `TJ` array's whole op makes its internal kerns irrelevant, and a pre-existing pinned test — `test_text_commit_textwriter_zorder.py::test_deleting_show_op_moves_later_text_then_erase_compensates_exactly` — erases one). `'`/`"` refused by both. See PITFALLS.
- [x] **Halo semantics under growth (resolved via option b, blankness proof):** `verify.growth_zone_is_uniform` proves the growth zone (declared minus source, the right-side band) is one flat pre-edit color before admitting growth — at `engine.prepare` (scratch), `engine.commit` (live, load-bearing re-check — annotation appearance content isn't fingerprint-covered), and preview (scratch, pre-splice). V0c/V0d widen to the declared region (target ∪ growth) for extraction/raster checks; span-origin exclusion stays on the source bbox. New known-limitation entries in PITFALLS (narrow-zone AA false positive, widened-clip false-reject, vertical-ink exposure — all safe-direction).
- [ ] **Fallback `target_bbox` still assumes left-to-right text in page space** (unchanged by Slice 1): the *shape* is still wrong on `/Rotate 90`/`270` pages independent of scale — box is built axis-aligned while glyphs run vertically in page space, so V0c would false-reject on Tier 0/shrink edits located on a rotated page (Slice 1's growth-direction guard only refuses *growth* on such pages; it does not fix the underlying bbox shape). Newly confirmed empirically (2026-08-09, see PITFALLS): `page.transformation_matrix` does not even reflect `/Rotate` on the installed PyMuPDF, so any fix here must read `page.rotation` directly, not infer rotation from the matrix. Fix still needs a rect-in-user-space-then-transform formulation plus a `/Rotate` fixture.
- [ ] **`TEXT_COMMIT_TELEMETRY` is parsed into `TextCommitSettings.telemetry` but read nowhere** (measurement stage, 2026-08-01) — wire it to the shadow-mode logging path or remove the flag; a settings field that silently does nothing invites false confidence that telemetry was enabled
- [ ] **D4 — `_ocg_membership_lost` tri-state, ALL failure paths** (`verify.py:341-379`: locked probe plus every raised-exception branch currently return `False`, which the caller records as `ocg_membership_preserved`). Still not needed for Slice 1 as shipped: Tier 1's production verifier (`verify_tier1_commit`) never calls `_ocg_membership_lost`/`verify_tier1_strategy` at all — no OCG claim is ever made (D8: transplant inherits OCG by byte position, V0a's byte-identity-outside-range proof already covers it). Required only if a future slice adds an OCG probe to the production path
- [ ] **TJ arrays — cheap half only, gated on the binding-survival measurement:** still refused in Slice 1 as planned (pivot condition met: 0.59% binding survival). admit whole-array targets via `build_transplant_replacement` (sound by construction — an accepted target always covers the entire operator, so no unedited glyph sits inside the replaced range), preserving leading/trailing kerns; never the unsound `array_item_count==1` rule. Do NOT build in-array splicing with kern rebalancing — it serves only substring targets every tier refuses. *(Gate measured 2026-08-01: binding survival is 0.59% show-weighted — 146 of 24,665 TJ shows are kernless; 99.4% can never byte-bind. On this corpus the pivot condition "defer if binding survival is negligible" is met; revisit only if a representative corpus disagrees or binding stops demanding byte equality)*
