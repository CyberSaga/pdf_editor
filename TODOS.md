...
- [ ] **Same-line successor merges into the target's own rawdict span** (new deviation, 2026-08-02) — two Slice 1 fixtures (neighbour-word and single-narrow-glyph growth-refusal) required a char-level `_target_bbox(page, probe)` helper instead of span-level `span["bbox"]` when the target and its same-line successor merge into one rawdict span without an intervening Tf/Tm/T*. Deviation recorded in test_scripts/test_text_commit_tier1_slice1.py; cross-reference from PITFALLS.md if regenerated.
